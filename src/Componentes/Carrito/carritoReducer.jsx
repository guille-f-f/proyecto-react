...
export const carritoReducer = (state, action) => {
  switch(action.type){
    ...
    case TYPES.ADD_TO_CART:
      const productoAgregado = state.productos.find(producto => producto.id === action.payload)
      const itemEnCarrito = state.carrito.find(item => item.id === productoAgregado.id)
    ...
    case TYPES.REMOVE_ITEM: 
      const unidadAEliminar = state.carrito.find(item => item.id === action.payload)
      ...
      : {
        ...state,
        carrito: state.carrito.filter(item => item.id !== unidadAEliminar.id)
      }

    case TYPES.REMOVE_ALL_ITEM: 
      const itemAEliminar = state.carrito.find(item => item.id === action.payload)
      return {
        ...state,
